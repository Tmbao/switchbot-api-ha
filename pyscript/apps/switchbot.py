...
def gen_dev_uid(dev: dict):
    ...
    return (
        prefix
        + re.sub(r"[^0-9a-z]+", "_", str(dev.get(KEY_RMT_DEV_TYPE)).lower())
        + "_"
        + str(dev.get(KEY_DEV_ID)[-4:])
    )
...
def switchbot_refresh_devices():
    ...
    cloud_devices = body.get("deviceList")
    if infrared_devices is None and cloud_devices is None:
        return None
    clear_existing()
    for dev in infrared_devices:
        log.warning(
            f"Adding Switchbot Device {dev.get(KEY_DEV_NAME)} [{dev.get(KEY_RMT_DEV_TYPE)}] -> {dev.get(KEY_DEV_ID)}"
        )
        dev["friendly_name"] = gen_dev_name(dev)
        dev["icon"] = gen_icon(dev.get(KEY_RMT_DEV_TYPE))
        state.set(
            f"{DOMAIN}.{gen_dev_uid(dev)}",
            value=dev.get(KEY_DEV_ID),
            new_attributes=dev,
        )

    for dev in cloud_devices:
        log.warning(
            f"Adding Switchbot Device {dev.get(KEY_DEV_NAME)} [{dev.get(KEY_DEV_TYPE)}] -> {dev.get(KEY_DEV_ID)}"
        )
        dev["friendly_name"] = gen_dev_name(dev)
        dev["icon"] = gen_icon(dev.get(KEY_DEV_TYPE))
        state.set(
            f"{DOMAIN}.{gen_dev_uid(dev)}",
            value=dev.get(KEY_DEV_ID),
            new_attributes=dev,
...
def switchbot_hvac(device, temperature, mode, fan_speed, state):
    ...
    command_execute(
        ...
        deviceId,
        "setAll",
        parameter=f"{temperature},{mode},{fan_speed},{state}",
    )
...
@service
def switchbot_generic_command(
    device=None, command=None, parameter=None, commandType=None
):
    """yaml
